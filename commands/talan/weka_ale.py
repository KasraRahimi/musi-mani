from interactions import slash_command, SlashContext, ActionRow, Button, ButtonStyle, check, is_owner
...
async def get_owner_confirmation(ctx: SlashContext) -> None | str:
    msg = await ctx.send("sina pali e ni la mani ale pi jan ale li weka. ona li weka la ona li ken ala kama sin\nsina __**wile ala wile**__ pali e ni?", components=ACTION_ROW)
...
async def weka_ale(ctx: SlashContext):
    ...
    match owner_choice:
        ...
        case None:
            await ctx.send("mi sukumen e toki sina, taso sina toki ala. ni la mi pini e ken sina.")
