...
from commands.games.constants import (
    COMMAND_NAME,
    COMMAND_DESCRIPTION,
    BET_OPTION,
    can_player_bet,
...
async def get_initial_message(ctx: SlashContext) -> Message:
    ...
    message = ("### Coin Toss", f"<@{user_id}>'s game", "", "Pick an outcome.")
    message = "\n".join(message)
...
async def get_player_choice(ctx: SlashContext, msg: Message) -> Choice | None:
    ...
    try:
        user_component = await bot.wait_for_component(
            components=action_row, timeout=30, check=same_user_check
        )
...
async def edit_message_during_coin_flip(
    ctx: SlashContext, msg: Message, delay_seconds: float = 1.0
) -> None:
    sleep_time = delay_seconds / 3
    for i in range(1, 4):
        await ctx.edit(
            msg, content=get_flipping_message(str(ctx.author.id), i), components=[]
        )
        await sleep(sleep_time)


async def handle_end_game(
    ctx: SlashContext, msg: Message, player_choice: Choice, outcome: Choice, bet: int
) -> None:
